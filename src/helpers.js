...
hbs.registerHelper("listarUsuariosCurso", (idCurso) => {
  
  let listaCursos = require("../cursos.json");
  let cursoUsuarios = listaCursos.find(curso => curso.id == idCurso);
  let usuariosCursos = cursoUsuarios.usuarios;
  
  let texto = "";
  usuariosCursos.map(usuario => {
    texto += `<tr>               
              ...
              <td>
                <a href="/eliminar-usuario?documento=${usuario.documento}&idcurso=${idCurso}">Eliminar</a>
              </td>
            </tr>
     `;
  })
